marking-UndershirtDefault-undershirt = Майка
marking-UndershirtDefault = Футболка (за замовчуванням)
marking-UndershirtRolled-rolled_undershirt = Майка
marking-UndershirtRolled = Нижня сорочка (в рулоні)
marking-UndershirtSleeveless-sleeveless = Майка
marking-UndershirtSleeveless = Майка
marking-UndershirtGrossSleeveless-gross_sleeveless = Майка
marking-UndershirtGrossSleeveless = Майка (брутто)
marking-UndershirtRolledSleeveless-rolled_undershirt_sleeveless = Майка
marking-UndershirtRolledSleeveless = Майка (рулонна)
marking-UndershirtNanotrasen-nanotrasen = Майка
marking-UndershirtNanotrasen = Натільна сорочка (Нанотрасен)
marking-UndershirtBinder-binder = Палітурка
marking-UndershirtBinder = Палітурка
marking-UndershirtBraClassic-classic = Бюстгальтер
marking-UndershirtBraClassic = Бюстгальтер (класичний)
marking-UndershirtBraSports-sports = Бюстгальтер
marking-UndershirtBraSports = Бюстгальтер (спортивний)
marking-UndershirtBraStrapless-strapless = Бюстгальтер
marking-UndershirtBraStrapless = Бюстгальтер (без бретелей)