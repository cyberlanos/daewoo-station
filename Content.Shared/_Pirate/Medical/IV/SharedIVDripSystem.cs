...
public abstract class SharedIVDripSystem : EntitySystem
{
    ...
    private void OnIVTargetCanDropTarget(Entity<IVDripTargetComponent> target, ref CanDropTargetEvent args)
    {
        var iv = args.Dragged;
        if (!TryComp(iv, out IVDripComponent? ivComp) || !InRange(iv, target, ivComp.Range))
            return;
    ...
    private void OnIVVerbs(Entity<IVDripComponent> iv, ref GetVerbsEvent<InteractionVerb> args)
    {
        ...
        args.Verbs.Add(new InteractionVerb
        {
            ...
            Text = Loc.GetString("iv-drip-verb-toggle-inject"),
        });
    }
    ...
    private void OnIVExamine(Entity<IVDripComponent> ent, ref ExaminedEvent args)
    {
        using (args.PushGroup(nameof(IVDripComponent)))
        {
            var injectingMsg = ent.Comp.Injecting
                ? "iv-drip-examine-injecting"
                : "iv-drip-examine-drawing";
            args.PushMarkup(Loc.GetString(injectingMsg, ("iv", ent.Owner)));

            var chemicalsMsg = Loc.GetString("iv-drip-examine-chemicals-none");
            ...
            {
                chemicalsMsg = Loc.GetString("iv-drip-examine-chemicals",
                    ("attached", packId),
            ...
            var attachedMsg = ent.Comp.AttachedTo is { } attached
                ? Loc.GetString("iv-drip-examine-attached", ("attached", attached))
                : Loc.GetString("iv-drip-examine-attached-none");
    ...
    private void OnBloodPackAfterInteract(Entity<BloodPackComponent> pack, ref AfterInteractEvent args)
    {
        ...
        {
            DetachPack(pack, user, false, true);
            return;
        }

        if (user == target)
        {
            _popup.PopupClient(Loc.GetString("iv-drip-blood-pack-cannot-self"), user, user);
            return;
        }
        ...
        {
            var selfPoke = Loc.GetString("iv-drip-blood-pack-poke-self", ("pack", pack.Owner), ("target", target));
            var othersPoke = Loc.GetString("iv-drip-blood-pack-poke-others",
                ("user", user),
                ("pack", pack.Owner),
                ("target", target));
    ...
    {
        DetachPack(pack, args.User, true, true);
    }
    ...
    private void OnBloodPackExamine(Entity<BloodPackComponent> pack, ref ExaminedEvent args)
    {
        using (args.PushGroup(nameof(BloodPackComponent)))
        {
            ...
            if (_solutionContainer.TryGetSolution(pack.Owner, pack.Comp.Solution, out _, out var solution))
                args.PushMarkup(Loc.GetString("iv-drip-blood-pack-contains", ("units", solution.Volume.Int())));
        }
    }
    ...
    {
        if (!HasComp<IVDripTargetComponent>(to) || !InRange(iv, to, iv.Comp.Range))
            return;
    ...
    {
        if (!HasComp<IVDripTargetComponent>(to) || !InRange(pack, to, pack.Comp.Range))
            return;
    ...
    private void ToggleInject(EntityUid iv, ref bool injecting, EntityUid user)
    {
        ...
        var msg = injecting
            ? Loc.GetString("iv-drip-now-injecting")
            : Loc.GetString("iv-drip-now-taking");
    ...
    protected void UpdatePackVisuals(Entity<BloodPackComponent> pack)
    {
        ...
        if (_containers.TryGetContainingContainer((pack.Owner, null, null), out var container) &&
    ...
    {
        // The client does not always know about solutions.
        if (_net.IsClient)
        {
            UpdateIVAppearance(iv);
            return;
        ...
        bool predict)
    {
        ...
        var message = Loc.GetString("iv-drip-rip", ("target", attached));
        if (predict)
        {
    ...
    private void AttachFeedback(EntityUid iv, EntityUid user, EntityUid to, bool injecting)
    {
        ...
        var selfMessage = "iv-drip-attach-self-drawing";
        var othersMessage = "iv-drip-attach-others-drawing";
        if (injecting)
        {
            selfMessage = "iv-drip-attach-self-injecting";
            othersMessage = "iv-drip-attach-others-injecting";
        }

        _popup.PopupClient(Loc.GetString(selfMessage, ("iv", iv), ("target", to)), to, user);

        var others = Filter.PvsExcept(user);
        _popup.PopupEntity(Loc.GetString(othersMessage, ("iv", iv), ("user", user), ("target", to)),
            to,
            others,
            true);
    }
    ...
    {
        var selfMessage = Loc.GetString("iv-drip-detach-self", ("iv", iv), ("target", attached));
        if (predict)
        ...
        _popup.PopupEntity(Loc.GetString("iv-drip-detach-others", ("iv", iv), ("user", user), ("target", attached)),
            attached,
            others,
            true);
    }
