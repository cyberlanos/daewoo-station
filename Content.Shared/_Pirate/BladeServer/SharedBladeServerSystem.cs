...
namespace Content.Shared._Pirate.BladeServer;
...
public abstract partial class SharedBladeServerSystem : EntitySystem
{
    ...
    private void OnExamined(Entity<BladeServerRackComponent> entity, ref ExaminedEvent args)
    {
        ...
        {
            args.PushMarkup(Loc.GetString("pirate-blade-server-rack-examine-empty"));
            return;
        }
        ...
        foreach (var (idx, slot) in entity.Comp.BladeSlots.Index())
        {
            ...
            var slotText = Loc.GetString(
                "pirate-blade-server-rack-examine-slot",
                ("index", idx + 1),
        ...
        {
            args.PushMarkup(
                Loc.GetString("pirate-blade-server-rack-examine-single", ("slot", slots.Single()))
            );
            return;
        }
        ...
        {
            args.PushMarkup(Loc.GetString("pirate-blade-server-rack-examine-multiple-start"));
            foreach (var slot in slots)
            {
                args.PushMarkup(
                    Loc.GetString("pirate-blade-server-rack-examine-multiple-slot-line", ("slot", slot))
                );
            }
        }
    }
    ...
    /// re-constructed for some reason, this could perhaps return entities which previously existed in this rack. A
    ...
    )
    {
        ...
        foreach (var idx in Enumerable.Range(0, entity.Comp.NumSlots))
        {
            var slot = new ItemSlot
            {
                ...
                LockedFailPopup = "pirate-blade-server-rack-slot-locked-fail",
                WhitelistFailPopup = "pirate-blade-server-rack-slot-whitelist-fail",
            };
    ...
    private void OnBladeServerFrameInteractUsing(Entity<BladeServerFrameComponent> entity, ref InteractUsingEvent args)
    {
        ...
        {
            _popup.PopupPredictedCursor(Loc.GetString("pirate-blade-server-frame-incompatible-board"), args.User);
        }
    }

    private void OnBladeServerBoardExamined(Entity<BladeServerBoardComponent> entity, ref ExaminedEvent args)
    {
        args.PushMarkup(Loc.GetString("pirate-blade-server-board-compatible-hint"));
    }
}
