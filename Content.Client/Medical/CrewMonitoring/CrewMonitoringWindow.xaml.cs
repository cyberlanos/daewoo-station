...
public sealed partial class CrewMonitoringWindow : FancyWindow
{
    ...
    private EntityUid? _zLevelRootGrid; // Pirate: multiz
    ...
    private EntityCoordinates CoordinatesToLocal(EntityCoordinates refCoords)
    {
        ...
        {
            var mapCoordinates = _transformSystem.ToMapCoordinates(refCoords, false); // Pirate: multiz
            if (_entManager.TryGetComponent<TransformComponent>(NavMap.MapUid.Value, out var xform) && // Pirate: multiz
                mapCoordinates.MapId == xform.MapID) // Pirate: multiz
                return _transformSystem.WithEntityId(refCoords, NavMap.MapUid.Value); // Pirate: multiz
        }

        return refCoords; // Pirate: multiz
    }
